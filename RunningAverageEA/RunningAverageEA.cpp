...
//          http://stromputer.googlecode.com/svn-history/r74/trunk/Arduino/Libraries/RunningAverageEA/RunningAverageEA.cpp
...
RunningAverageEA::RunningAverageEA(int n)
{
    _size = n;
...
RunningAverageEA::~RunningAverageEA()
{
    if (_ar != NULL) free(_ar);
}

// resets all counters
void RunningAverageEA::clear()
{
    _cnt = 0;
    _idx = 0;
    _sum = 0.0;
...
void RunningAverageEA::addValue(float f)
{
    if (_ar == NULL) return;
    _sum -= _ar[_idx];
    _ar[_idx] = f;
...
float RunningAverageEA::getAverage()
{
...
float RunningAverageEA::getElement(uint8_t idx)
{
...
void RunningAverageEA::fillValue(float value, int number)
{
    clear();
...
float RunningAverageEA::getStandardDeviation()
{
